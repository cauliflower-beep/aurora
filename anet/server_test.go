...
func ClientTest(i uint32) {
	...
	for {
		...
		msg, _ := dp.Pack(apack.NewMsgPackage(i, []byte("client test message")))
...
func (d *DemoPacket) Pack(msg aiface.IMessage) ([]byte, error) {
	time.Sleep(time.Second * 1)
	return d.DataPack.Pack(msg)
}

func (br *CloseConnectionBeforeSendMsgRouter) Handle(req aiface.IRequest) {
...
func TestCloseConnectionBeforeSendMsg(t *testing.T) {
	...
	go func() {
		...
		pack, _ := dp.Pack(apack.NewMsgPackage(1, []byte(msg)))
		_, _ = conn.Write(pack)
		fmt.Println("send: ", msg)
