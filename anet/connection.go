...
type Connection struct {
	...
	connManager aiface.IConnManager
	//当前连接创建时Hook函数
	...
	//断粘包解码器
	lengthFieldDecoder aiface.ILengthField
	//心跳检测器
...
func newServerConn(server aiface.IServer, conn net.Conn, connID uint64) aiface.IConnection {
	//初始化Conn属性
	c := &Connection{
		conn:        conn,
		connID:      connID,
	...
	if lengthField != nil {
		c.lengthFieldDecoder = acode.NewLengthFieldFrameDecoderByLengthField(*lengthField)
	}
...
func newClientConn(client aiface.IClient, conn net.Conn) aiface.IConnection {
	c := &Connection{
		conn:        conn,
...
func (c *Connection) StartWriter() {
	alog.Ins().InfoF("Writer Goroutine is running")
	defer alog.Ins().InfoF("%s [conn Writer exit!]", c.RemoteAddr().String())
		...
		case data, ok := <-c.msgBuffChan:
			if ok {
				...
				if _, err := c.conn.Write(data); err != nil {
					alog.Ins().ErrorF("Send Buff Data error:, %s Conn Writer exit", err)
					return
				}
				...
				alog.Ins().ErrorF("msgBuffChan is Closed")
				break
			}
		case <-c.ctx.Done():
			return
...
func (c *Connection) StartReader() {
	alog.Ins().InfoF("[Reader Goroutine is running]")
	defer alog.Ins().InfoF("%s [conn Reader exit!]", c.RemoteAddr().String())
	defer c.Stop()
	...
	for {
		...
		default:
			...
			buffer := make([]byte, aconf.GlobalObject.IOReadBuffSize)
			n, err := c.conn.Read(buffer[:])
			if err != nil {
				alog.Ins().ErrorF("read msg head [read datalen=%d], error = %s", n, err)
				return
			}
			alog.Ins().DebugF("read buffer %s \n", hex.EncodeToString(buffer[0:n]))
			...
			if c.lengthFieldDecoder != nil {
				...
				for _, bytes := range bufArrays {
					...
					msg := apack.NewMessage(uint32(len(bytes)), bytes)
					//得到当前客户端请求的Request数据
					req := NewRequest(c, msg)
					c.msgHandler.Decode(req)
				}
			} else {
				msg := apack.NewMessage(uint32(n), buffer[0:n])
				//得到当前客户端请求的Request数据
				req := NewRequest(c, msg)
				c.msgHandler.Decode(req)
			}
...
func (c *Connection) Send(data []byte) error {
	...
	if err != nil {
		alog.Ins().ErrorF("SendMsg err data = %+v, err = %+v", data, err)
		return err
	}
...
func (c *Connection) SendToQueue(data []byte) error {
	...
	if c.msgBuffChan == nil {
		c.msgBuffChan = make(chan []byte, aconf.GlobalObject.MaxMsgChanLen)
	...
	if data == nil {
		alog.Ins().ErrorF("Pack data is nil")
...
func (c *Connection) SendMsg(msgID uint32, data []byte) error {
	...
	//将data封包，并且发送
	msg, err := c.packet.Pack(apack.NewMsgPackage(msgID, data))
	if err != nil {
		alog.Ins().ErrorF("Pack error msg ID = %d", msgID)
	...
	if err != nil {
		alog.Ins().ErrorF("SendMsg err msg ID = %d, data = %+v, err = %+v", msgID, string(msg), err)
		return err
	}
...
func (c *Connection) finalizer() {
	...
	alog.Ins().InfoF("Conn Stop()...ConnID = %d", c.connID)
}

// callOnConnStart 调用连接OnConnStart Hook函数
func (c *Connection) callOnConnStart() {
	if c.onConnStart != nil {
		alog.Ins().InfoF("ZINX CallOnConnStart....")
		c.onConnStart(c)
	}
}
...
func (c *Connection) callOnConnStop() {
	if c.onConnStop != nil {
		alog.Ins().InfoF("ZINX CallOnConnStop....")
		c.onConnStop(c)
	}
}
...
func (c *Connection) IsAlive() bool {
	...
	return time.Now().Sub(c.lastActivityTime) < aconf.GlobalObject.HeartbeatMaxDuration()
}
...
func (c *Connection) SetHeartBeat(checker aiface.IHeartbeatChecker) {
	c.hc = checker
}
