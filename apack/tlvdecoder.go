...
func NewTLVDecoder() aiface.IDecoder {
	return &TLVDecoder{}
}

func (this *TLVDecoder) GetLengthField() *aiface.LengthField {
	// +---------------+---------------+---------------+
	// |    Tag        |     Length    |     Value     |
...
func (this *TLVDecoder) Intercept(chain aiface.Chain) aiface.Response {
	...
	if request != nil {
		...
		case aiface.IRequest:
			...
			if iMessage != nil {
				...
				alog.Ins().DebugF("TLV-RawData size:%d data:%s\n", len(data), hex.EncodeToString(data))
				datasize := len(data)
				_data := TLVDecoder{}
				if datasize >= TLV_HEADER_SIZE {
					...
					//alog.Ins().DebugF("TLV-DecodeData size:%d data:%+v\n", unsafe.Sizeof(data), _data)
				}
			}
		}
	}
