...
type ConnIDMap map[uint64]aiface.IConnection
...
func NewZNotify() aiface.Inotify {
	return &notify{
		cimap: make(map[uint64]aiface.IConnection, 5000),
	}
}
...
func (n *notify) SetNotifyID(Id uint64, conn aiface.IConnection) {
	n.Lock()
	defer n.Unlock()
	n.cimap[Id] = conn
}

func (n *notify) GetNotifyByID(Id uint64) (aiface.IConnection, error) {
	n.RLock()
	defer n.RUnlock()
	Conn, ok := n.cimap[Id]
	if !ok {
...
func (n *notify) NotifyAll(MsgId uint32, data []byte) error {
	...
	for Id, v := range n.cimap {
		...
		if err != nil {
			alog.Ins().ErrorF("Notify to %d err:%s \n", Id, err)
			return err
		}
	}
	return nil
...
func (n *notify) notifyAll2(MsgId uint32, data []byte) error {
	conns := make([]aiface.IConnection, 0, len(n.cimap))
	n.RLock()
